...
# Split into columns
if test:
    index, time, v_comm, raw_ir1, raw_ir2, raw_ir3, raw_ir4, \
...
for i in range(0, len(index)-1):
    ...
    postXL.append(postX)
    postVarL.append(postVar)
    # Repeat with updated initial guesses
    initialX = postX
    initialVar = postVar
...
if not test:
    error = []
    for item1, item2 in zip(dist, postXL):
        error.append(item1-item2)
    print("Mean error in model:", np.mean(error))

# Derive std dev from Variance
std = []
for Var in postVarL:
    std.append(np.sqrt(Var))

# Plotting range to get better resolution on areas
m = 0
M = -1 #len(time)
x = time[m:(M-1)]
...
# Plot 2 Showing the position Std dev and Gains
fig, axes = subplots(3)
